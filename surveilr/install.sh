...
case "$PLATFORM" in
    ...
    *"cygwin"*|*"mingw"*|*"msys"*) BUILD="pc-windows-msvc.zip" ;;
...
DOWNLOAD_URL=$(curl -s $API_URL | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' | xargs -I {} echo "https://github.com/$REPO_OWNER/$REPO_NAME/releases/download/{}/surveilr_{}_x86_64-${BUILD}") || { echo "Failed to construct download URL" | tee -a error.log; exit 1; }
